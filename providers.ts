...
export const MODELS: ProviderModel[] = [
  // Groq
  { id: 'llama3-8b-8192',       name: 'Llama3 8B',        provider: 'groq',      free: true,  contextK: 8,    quality: 3, speed: 5 },
  { id: 'llama3-70b-8192',      name: 'Llama3 70B',       provider: 'groq',      free: true,  contextK: 8,    quality: 4, speed: 4 },
  { id: 'mixtral-8x7b-32768',   name: 'Mixtral 8x7B',     provider: 'groq',      free: true,  contextK: 32,   quality: 4, speed: 3 },
  // Gemini
...
export async function autoRoute(query: string, hasImage: boolean): Promise<ProviderModel> {
  ...
  if (task === 'fast') {
    if (available.includes('groq'))      return MODELS.find(m => m.id === 'llama3-8b-8192')!;
    if (available.includes('gemini'))    return MODELS.find(m => m.id === 'gemini-2.0-flash')!;
  ...
  if (available.includes('groq'))      return MODELS.find(m => m.id === 'llama3-70b-8192')!;
  if (available.includes('gemini'))    return MODELS.find(m => m.id === 'gemini-2.0-flash')!;
